...
# ===== 各ページのキーワード出現回数を取得 =====
tobus=$(curl -sL "$TOBUS_URL" 2>/dev/null | grep -coE "$KW" || echo 0)
metro=$(curl -sL "$METRO_URL" 2>/dev/null | grep -coE "$KW" || echo 0)
jr=$(curl -sL "$JR_URL" 2>/dev/null | grep -coE "$KW" || echo 0)
...
jq -n \
  ...
  --argjson mt "$metro" \
  --argjson jr "$jr" \
  --argjson al "$(echo "$new_alerts" | jq -c .)" \
